...
function main() {
  ...
  for (var i = 0, len = pions.length; i < len; i++) {
    pions[i].addEventListener("click", function() {
      ...
      if (!estValide(this)) {
        document.getElementById("deselects").currentTime = 0
        document.getElementById("deselects").play()
        afficheur.sendMessage(
        ...
        document.getElementById("selects").currentTime = 0
        document.getElementById("selects").play()
        if (jeuEstFini) {
          afficheur.sendMessage(
            "Le joueur " +
              joueurs[tour] +
